...
function npmRunPath(options = {}) {
	...
	let cwdPath = path$c.resolve(cwdString);
	const result = [];

	while (previous !== cwdPath) {
		result.push(path$c.join(cwdPath, 'node_modules/.bin'));
		previous = cwdPath;
		cwdPath = path$c.resolve(cwdPath, '..');
	}

	// Ensure the running `node` binary is used.
	result.push(path$c.resolve(cwdString, execPath, '..'));

	return [...result, path_].join(path$c.delimiter);
}
...
const handleArguments = (file, args, options = {}) => {
	...
	if (process$2.platform === 'win32' && path$c.basename(file, '.exe') === 'cmd') {
		// #116
		args.unshift('/q');
	}
...
async function findUpMultiple(name, options = {}) {
	...
	const stopAt = path$c.resolve(directory, options.stopAt || root);
...
var isGlob = isGlob$1;
var pathPosixDirname = path$c.posix.dirname;
...
const parse$5 = (input, options = {}) => {
  ...
  while (index < length) {
    ...
    if (value === CHAR_DOT$2 && depth > 0 && block.commas === 0) {
...
braces$2.parse = (input, options = {}) => parse$4(input, options);
...
picomatch$5.scan = (input, options) => scan$2(input, options);
